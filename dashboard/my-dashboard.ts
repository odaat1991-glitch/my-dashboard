...
import { 
  ...
  Headphones, Radio, CloudRain, Maximize2
} from 'lucide-react';
...
const playNotificationSound = (volume = 0.5, type = 'complete') => {
  ...
  try {
    ...
    if (type === 'complete') {
      ...
      osc.frequency.linearRampToValueAtTime(783.99, ctx.currentTime + 0.2); // G5
...
class AmbientGenerator {
  ...
  // Generates a single chime sound
  playChime(volume) {
    if (!this.ctx || volume <= 0) return;
    const osc = this.ctx.createOscillator();
    const gain = this.ctx.createGain();
    
    // Pentatonic scale notes (C6, D6, E6, G6, A6)
    ...
    gain.gain.linearRampToValueAtTime(volume * 0.3, now + 0.05); // Soft attack
  ...
  stopAll() {
    Object.keys(this.nodes).forEach(type => {
      try { this.nodes[type].source.stop(); } catch(e){}
    });
        ...
        <div className="md:col-span-2 bg-white rounded-3xl shadow-sm border border-slate-100 p-8 flex flex-col items-center relative overflow-hidden">
          <div className="flex space-x-2 bg-slate-50 p-1.5 rounded-xl mb-8 relative z-10">
            <button 
              ...
              className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all duration-300 ${mode === 'work' ? 'bg-white text-indigo-600 shadow-sm scale-105' : 'text-slate-400 hover:text-slate-600'}`}
            >
              Deep Work
            </button>
            <button 
              onClick={() => switchMode('break')}
              className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all duration-300 ${mode === 'break' ? 'bg-white text-teal-600 shadow-sm scale-105' : 'text-slate-400 hover:text-slate-600'}`}
            >
              Rest
            </button>
          </div>
          ...
          <div className="relative mb-8 group cursor-default">
             <svg className="transform -rotate-90 w-80 h-80">
              ...
              <circle
                ...
                strokeLinecap="round"
                className={`transition-all duration-1000 ease-linear ${mode === 'work' ? 'text-indigo-500' : 'text-teal-500'}`}
              />
            </svg>
            
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
              <div className={`text-6xl font-mono font-bold tracking-tighter ${mode === 'work' ? 'text-indigo-900' : 'text-teal-900'}`}>
                {formatTime(timeLeft)}
              </div>
              <div className="text-slate-400 text-sm font-medium mt-2 tracking-widest uppercase">
          ...
          <div className="flex items-center space-x-6 relative z-10">
            <button 
              onClick={resetTimer}
              className="p-4 rounded-2xl bg-slate-50 text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors"
            >
              <RotateCcw size={24} />
            </button>
            ...
            <button 
              onClick={toggleTimer}
              className={`p-6 rounded-3xl transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-xl hover:shadow-2xl ${
                isActive 
                  ? 'bg-amber-50 text-amber-500 hover:bg-amber-100 ring-4 ring-amber-50' 
                  : mode === 'work' 
          ...
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-5xl font-bold">{dailyCount}</span>
                <span className="text-indigo-200 text-sm font-medium">sessions</span>
              </div>
              <div className="mt-4 bg-white/20 h-2 rounded-full overflow-hidden backdrop-blur-sm">
                <div 
                  ...
                  style={{ width: `${Math.min((dailyCount / 8) * 100, 100)}%` }}
                />
              </div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex-1">
            ...
            <div className="space-y-4">
              ...
              <input 
                ...
                className="w-full h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-indigo-500 hover:accent-indigo-600"
              />
...
// 3. KANBAN BOARD
const KanbanColumn = ({ title, status, tasks, user, onDrop, onDragStart, color }) => {
  ...
  return (
    <div 
      className={`flex-1 bg-slate-50/50 rounded-xl flex flex-col min-h-[500px] border-t-4 ${borderColor} shadow-[0_8px_30px_rgb(0,0,0,0.04)]`}
      onDragOver={handleDragOver}
      onDrop={(e) => onDrop(e, status)}
    >
      <div className={`p-4 rounded-t-sm ${headerBg} flex items-center justify-between border-b border-slate-200/50`}>
        <h3 className={`font-bold ${textColor} uppercase tracking-wide text-sm`}>
          {title}
        </h3>
        <span className="bg-white/80 backdrop-blur-sm text-slate-500 text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">
          {tasks.length}
        </span>
      </div>
      ...
      <div className="p-3 space-y-3 overflow-y-auto flex-1">
        {tasks.map(task => (
          <div 
            ...
            className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 cursor-move hover:shadow-lg hover:-translate-y-1 transition-all duration-200 group relative"
          >
            ...
            <div className="flex justify-end mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <button 
                  onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'kanban', task.id))}
...
const Kanban = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, 
      ...
      (error) => console.error("Kanban error", error)
    );
    return unsubscribe;
  }, [user]);
  ...
  const handleDrop = async (e, newStatus) => {
    ...
    if (id) {
      await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'kanban', id), { status: newStatus });
    }
  };
        ...
        <form onSubmit={addTask} className="flex gap-3">
          <div className="flex-1 relative">
             ...
             <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 rounded-lg border-transparent focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all text-slate-700"
            />
          </div>
          <button type="submit" className="bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700 font-medium shadow-sm transition-all hover:shadow-md">
            Add Task
          </button>
        </form>
      </div>
...
const Notes = ({ user }) => {
  ...
  const createNote = async () => {
    const ref = await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'notes'), {
      title: '',
      content: '',
      isCode: false,
  ...
  const updateActiveNote = async (field, value) => {
    ...
    await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'notes', activeNoteId), {
      [field]: value,
      updatedAt: serverTimestamp()
    });
  };
      ...
      <div className="w-64 bg-slate-50 border-r border-slate-200 flex flex-col">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-white">
          <h2 className="font-semibold text-slate-700 flex items-center gap-2">
            <FileText size={18} className="text-indigo-500"/> Library
          </h2>
          <div className="flex gap-1">
             <input 
              ...
              accept=".md,.txt" 
            />
            <button onClick={handleImportClick} className="p-1.5 hover:bg-slate-100 rounded-md text-slate-500 hover:text-indigo-600 transition-colors" title="Import Markdown">
              <Upload size={16} />
            </button>
            <button onClick={createNote} className="p-1.5 hover:bg-slate-100 rounded-md text-slate-500 hover:text-indigo-600 transition-colors">
              <Plus size={16} />
            </button>
          </div>
        </div>
        <div className="overflow-y-auto flex-1 p-2 space-y-1">
          {notes.map(note => (
            <div 
              ...
              className={`p-3 rounded-lg cursor-pointer group transition-all ${activeNoteId === note.id ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-600 hover:bg-white hover:shadow-sm'}`}
            >
              <div className="flex justify-between items-start">
                <h4 className={`font-medium truncate text-sm ${!note.title ? 'italic opacity-50' : ''}`}>
              ...
              <p className="text-[10px] text-slate-400 mt-1 flex items-center gap-1">
                {note.isCode ? <Code size={10} /> : <FileText size={10} />}
                {new Date(note.updatedAt?.seconds * 1000 || Date.now()).toLocaleDateString()}
              </p>
            </div>
          ))}
        </div>
            ...
            <div className="px-8 py-4 border-b border-slate-100 flex items-center justify-between">
               <div className="flex-1 mr-4">
                 <input 
                  ...
                  className="text-xl font-bold text-slate-800 outline-none w-full bg-transparent placeholder:text-slate-300"
               ...
               <div className="flex items-center gap-2">
                 ...
                 <button 
                  ...
                  className={`px-3 py-1.5 rounded-lg flex items-center gap-2 text-xs font-bold tracking-wide transition-colors ${activeNote.isCode ? 'bg-slate-800 text-green-400' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                >
                  {activeNote.isCode ? <><Code size={14} /> CODE</> : <><FileText size={14} /> TEXT</>}
                </button>
               </div>
            </div>
            <textarea 
              className={`flex-1 w-full p-8 outline-none resize-none leading-relaxed text-base ${activeNote.isCode ? 'font-mono bg-[#1e1e1e] text-[#d4d4d4]' : 'bg-white text-slate-600 selection:bg-indigo-100'}`}
              value={activeNote.content}
              onChange={(e) => updateActiveNote('content', e.target.value)}
              placeholder={activeNote.isCode ? '// Type code snippet here...' : 'Start writing...'}
...
const HabitTracker = ({ user }) => {
  ...
  const deleteHabit = async (id) => {
    if (window.confirm('Stop tracking this habit?')) {
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
        ...
        <form onSubmit={addHabit} className="flex-1 flex gap-2">
          <input 
            ...
            className="flex-1 px-4 py-2 bg-transparent outline-none text-lg text-slate-700 placeholder:text-slate-300"
          />
          <button type="submit" className="bg-slate-900 text-white px-6 py-2 rounded-xl hover:bg-slate-800 font-medium transition-colors">
            Add Habit
          </button>
        </form>
      </div>
                  ...
                  <tr key={habit.id} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                    <td className="p-6">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-xs text-slate-400">{Math.round(progress)}% weekly</span>
                      </div>
                    </td>
                    <td className="p-6">
                      <div className={`flex items-center gap-2 font-bold ${streak > 2 ? 'text-orange-500' : 'text-slate-400'}`}>
                        <Flame size={20} className={`${streak > 2 ? 'fill-orange-500 animate-pulse' : ''}`}/>
                        {streak} days
                      </div>
                    </td>
                    {dates.map(date => {
                      ...
                      return (
                        <td key={date} className="p-2 text-center">
                          <button 
                            onClick={() => toggleHabitDay(habit, date)}
                            className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${isCompleted ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200 scale-105' : 'bg-slate-100 text-slate-300 hover:bg-slate-200 hover:scale-105'}`}
                          >
                            {isCompleted ? <CheckSquare size={20} /> : <div className="w-3 h-3 rounded-sm bg-slate-300/50" />}
                          </button>
                        </td>
                      );
                    })}
                    <td className="p-2 text-center">
                      <button 
                        ...
                        className="p-2 text-slate-300 hover:text-red-400 hover:bg-red-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                      >
                        <Trash2 size={18} />
                      </button>
                    </td>
...
const FocusSounds = () => {
  ...
  return (
    <div className="h-full grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 flex flex-col">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-3">
             <Sliders size={24} className="text-indigo-600"/> Ambient Mixer
          </h2>
          <p className="text-slate-400 mt-2">Mix your own soundscape using generated noise colors.</p>
        </div>
        ...
        <div className="space-y-6 flex-1 overflow-y-auto pr-2">
           ...
           <div className="bg-slate-50 rounded-2xl p-6">
             <div className="flex justify-between mb-4">
                <span className="font-semibold text-slate-700 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-pink-400"></div> Rain (Pink Noise)
                </span>
                <span className="text-slate-400 font-mono text-sm">{Math.round(volumes.pink * 100)}%</span>
             </div>
             <input 
                type="range" min="0" max="0.5" step="0.01" 
                value={volumes.pink} 
                onChange={(e) => toggleVolume('pink', parseFloat(e.target.value))}
           ...
           <div className="bg-slate-50 rounded-2xl p-6">
             <div className="flex justify-between mb-4">
                <span className="font-semibold text-slate-700 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-indigo-400"></div> Wind Chimes
                </span>
                <span className="text-slate-400 font-mono text-sm">{Math.round(volumes.chimes * 100)}%</span>
             </div>
             <input 
                type="range" min="0" max="0.5" step="0.01" 
                value={volumes.chimes} 
                onChange={(e) => toggleVolume('chimes', parseFloat(e.target.value))}
           ...
           <div className="bg-slate-50 rounded-2xl p-6">
             <div className="flex justify-between mb-4">
                <span className="font-semibold text-slate-700 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-amber-700"></div> River (Brown Noise)
                </span>
                <span className="text-slate-400 font-mono text-sm">{Math.round(volumes.brown * 100)}%</span>
             </div>
             <input 
                type="range" min="0" max="0.5" step="0.01" 
                value={volumes.brown} 
                onChange={(e) => toggleVolume('brown', parseFloat(e.target.value))}
                className="w-full h-2 bg-slate-200 rounded-full appearance-none cursor-pointer accent-amber-700"
             />
           </div>
           ...
           <div className="bg-slate-50 rounded-2xl p-6">
             <div className="flex justify-between mb-4">
                <span className="font-semibold text-slate-700 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-slate-400"></div> Static (White Noise)
                </span>
                <span className="text-slate-400 font-mono text-sm">{Math.round(volumes.white * 100)}%</span>
             </div>
             <input 
                type="range" min="0" max="0.1" step="0.01" 
                value={volumes.white} 
                onChange={(e) => toggleVolume('white', parseFloat(e.target.value))}
      ...
      {/* Right: SoundCloud Player (Updated to Chillhop Essentials) */}
      <div className="flex flex-col gap-6">
         <div className="bg-black rounded-3xl shadow-xl overflow-hidden relative group h-[400px] border border-white/10">
            <iframe 
              ...
              frameBorder="no" 
              allow="autoplay" 
              src={`https://w.soundcloud.com/player/?url=${encodeURIComponent('https://soundcloud.com/chillhopdotcom/sets/lofihiphop')}&color=%236366f1&auto_play=false&hide_related=false&show_comments=false&show_user=true&show_reposts=false&show_teaser=true&visual=true`}
              className="h-full w-full"
              title="Chillhop SoundCloud"
            ></iframe>
         </div>
         ...
         <div className="bg-indigo-600 rounded-3xl p-8 text-white shadow-lg flex-1 flex flex-col justify-center items-center text-center relative overflow-hidden">
             ...
             <div className="relative z-10">
               <Headphones size={48} className="mb-4 mx-auto opacity-80" />
               <h3 className="text-2xl font-bold mb-2">Focus Mode Active</h3>
               <p className="text-indigo-200 max-w-xs mx-auto">
                 Combine the ambient noise on the left with the music above for deep focus.
               </p>
             </div>
         </div>
      </div>
...
const App = () => {
  ...
  // Persistent Pomodoro State (Lifted Up)
  const WORK_TIME = 25 * 60;
  const BREAK_TIME = 5 * 60;
  ...
  // Pomodoro Logic (Moved to App Level)
  useEffect(() => {
    if (!user) return;
  ...
  const handleComplete = async () => {
    ...
    if (mode === 'work') {
      ...
      if (user) {
        await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'pomodoro_sessions'), {
  ...
  const toggleTimer = () => {
    if (!isActive) playNotificationSound(volume, 'click');
  ...
  if (!user) return <div className="h-screen flex items-center justify-center">Please refresh to sign in.</div>;
    ...
    <div className="flex h-screen bg-[#f8fafc] font-sans text-slate-900 overflow-hidden selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <div className="w-24 bg-white flex flex-col items-center py-8 space-y-8 shadow-2xl z-20 border-r border-slate-100">
        ...
        <div className="mt-auto">
           <div className="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-colors cursor-pointer shadow-sm" title={`User: ${user.uid}`}>
             <User size={20} />
           </div>
        </div>
      </div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-slate-50/50">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 flex items-center px-10 justify-between sticky top-0 z-10">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 capitalize tracking-tight">{activeTab === 'sounds' ? 'Focus Sounds' : activeTab}</h1>
            <p className="text-xs text-slate-400 font-medium mt-1"> Productivity Dashboard v2.1</p>
          </div>
          <div className="flex items-center gap-3 text-xs font-semibold bg-slate-100 px-3 py-1.5 rounded-full text-slate-500">
        ...
        {/* Persistent Mini Timer (Visible when NOT on Pomodoro tab) */}
        {activeTab !== 'pomodoro' && (
          <MiniTimer 
            timeLeft={timeLeft}
            isActive={isActive}
            mode={mode}
            toggleTimer={toggleTimer}
            onExpand={() => setActiveTab('pomodoro')}
          />
        )}
      </main>
    </div>
...
const NavButton = ({ icon: Icon, label, active, onClick }) => (
  <button 
    onClick={onClick}
    className={`w-full aspect-square flex flex-col items-center justify-center rounded-2xl transition-all duration-300 group relative ${active ? 'bg-indigo-50 text-indigo-600 shadow-inner' : 'text-slate-400 hover:bg-white hover:shadow-md hover:text-slate-600 hover:-translate-y-0.5'}`}
  >
    ...
    <span className="text-[10px] font-bold tracking-wide absolute -bottom-8 opacity-0 group-hover:opacity-100 transition-all bg-slate-800 text-white px-2 py-1 rounded-lg z-50 pointer-events-none shadow-xl translate-y-2 group-hover:translate-y-0">
      {label}
    </span>
  </button>
);
